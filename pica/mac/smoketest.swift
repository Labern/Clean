// Headless verification of the BUILT INT.:EXT..app bundle — loads the packaged web app through
...
let buildDir = ProcessInfo.processInfo.environment["PICA_BUILD_DIR"].map { URL(fileURLWithPath: $0) }
    ?? (FileManager.default.fileExists(atPath: "/tmp/pica-build/INT.:EXT..app/Contents/Resources/web/index.html")
        ? URL(fileURLWithPath: "/tmp/pica-build") : here.appendingPathComponent("build"))
let webRoot = buildDir.appendingPathComponent("INT.:EXT..app/Contents/Resources/web")
guard FileManager.default.fileExists(atPath: webRoot.appendingPathComponent("index.html").path) else {
    print("FAIL: build/INT.:EXT..app not found — run ./build.sh first"); exit(1)
}
...
func finishAll() {
    print(failures == 0 ? "\nINT.:EXT..app verified — the bundle runs standalone" : "\n\(failures) FAILURE(S)")
    exit(failures == 0 ? 0 : 1)
}
